...
csv = pd.read_csv("LCB-WCX.csv",index_col=0)
...
#     sl = SymbolLearning(loop ='MultiMutateLoop',pset=pset0, gen=20, pop=3000, hall=1, batch_size=40, re_hall=5,
...
#                         re_Tree=0, random_state=0, verbose=True,
#                         add_coef=True, inter_add=True, out_add=True, cal_dim=False,
#                         personal_map=False)
#
#     sl.fit()
...
#     plt.show()
    ...
    y_simple =  10.61 - 1.958 * np.exp(- 289.7 * X[:,1] / (X[:,0] ** 2))

    p = BasePlot(font=None)
    p.scatter(y, y_simple, strx='Experimental $E_{gap}$', stry='Calculated $E_{gap}$')
    import matplotlib.pyplot as plt
    score_simple = r2_score(y, y_simple)
    plt.show()
